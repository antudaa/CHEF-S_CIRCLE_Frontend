          ...
          <div className="relative w-full mt-4 max-w-xl mx-auto">
            {files.length > 0
              ...
              : (
                <motion.div
                  ...
                  className={cn(
                    "relative group-hover/file:shadow-2xl z-40 bg-white dark:bg-neutral-900 flex items-center justify-center h-20 mt-4 w-full max-w-[6rem] mx-auto rounded-md",
            ...
            {!files.length && (
              <motion.div
                ...
                className="absolute opacity-0 border border-dashed border-sky-400 inset-0 z-30 bg-transparent flex items-center justify-center h-20 mt-4 w-full max-w-[6rem] mx-auto rounded-md"
              />
            )}
          </div>
        </div>
...
export function GridPattern() {
  ...
  const rows = 11;
  return (
    <div className="flex bg-gray-100 dark:bg-neutral-900 flex-shrink-0 flex-wrap justify-center items-center gap-x-px gap-y-px scale-95">
      {Array.from({ length: rows }).map((_, row) => (
        <React.Fragment key={row}>
          {Array.from({ length: columns }).map((_, col) => (
            <div
              key={`${col}-${row}`}
              className={`w-6 h-6 flex flex-shrink-0 rounded-[2px] ${
                (row * columns + col) % 2 === 0
