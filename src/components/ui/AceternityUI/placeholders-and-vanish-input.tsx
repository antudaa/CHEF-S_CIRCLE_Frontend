...
export function PlaceholdersAndVanishInput({
  ...
  const newDataRef = useRef<{ x: number; y: number; r: number; color: string }[]>([]);  // Fix for `any[]`
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    vanishAndSubmit();
    onSubmit(e);  // Removed unnecessary `onSubmit &&` check
  };
      ...
      <input
        onChange={(e) => {
          if (!animating) {
            ...
            onChange?.(e); // Use optional chaining to avoid the error
          }
        }}
